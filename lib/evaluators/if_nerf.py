...
from lib.utils.ssim import ssim as ssim_m
import torch
...
class Evaluator:
    ...
    def ssim_metric(self, img_pred, img_gt, batch):
        ...
        # ssim = ssim_m(torch.from_numpy(img_pred),torch.from_numpy(img_gt)).mean().double()
    ...
    def evaluate(self, output, batch):
        ...
        rgb_pred = rgb_pred * batch['msk'][0].unsqueeze(-1).detach().cpu().numpy() #* batch['msk'][0].unsqueeze(-1).detach().cpu().numpy()
        rgb_gt = rgb_gt * batch['msk'][0].unsqueeze(-1).detach().cpu().numpy()
        ...
        rgb_pred = img_pred
        rgb_gt = img_gt
        ssim, psnr = self.ssim_metric(rgb_pred, rgb_gt, batch)
        self.psnr.append(psnr)
